...
describe("versioned Sporttery calculator", () => {
  test.each([
    ...
    (play, maximumPassSize) => {
      expect(SPORTTERY_RULES_V1.plays[play].maximumPassSize).toBe(
        maximumPassSize,
      );
      expect(
  ...
  test("simple passes from 2x1 through 8x1 expand once", () => {
    for (let size = 2; size <= 8; size += 1) {
      const result = calculateTicket({
        legs: Array.from({ length: size }, (_, index) =>
          leg(`m-${index}`, "had", [1.5]),
        ),
        passTypes: [`${size}x1`],
      } as CalculationInput);
  ...
  test("rounds each expanded winning bet half-even to cents", () => {
    expect(
      calculateTicket({ legs: [leg("a", "had", [1.0025])], passTypes: ["1x1"] })
        .maximumBonus,
    ).toBe(2);
    expect(
      calculateTicket({ legs: [leg("a", "had", [1.0075])], passTypes: ["1x1"] })
        .maximumBonus,
    ).toBe(2.02);
  });
  ...
  test("rejects combinations above the strictest selected play limit", () => {
    expect(() =>
      calculateTicket({
        legs: [leg("a", "crs"), leg("b"), leg("c"), leg("d"), leg("e")],
        passTypes: ["5x1"],
      }),
