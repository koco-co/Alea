...
本文档已同步当前实现，并用官方文档核验了 Supabase、Celery、Redis、Docker Compose、uv、Bun、Next.js 以及 AI Provider 的关键技术边界。本文档中的本地验证证据不等同于生产容量或 staging 证明；真实 Provider 额度、授权历史数据与生产恢复演练仍需在对应环境完成。

未取得授权数据时只允许使用明确标记的人工 fixture；任何 fixture、fake provider 或静态页面都不能作为真实生产数据或 AI 成功证据。
...
生产 Worker 必须显式设置 `ALEA_PHASE_EXECUTOR_FACTORY=app.workers.production_executor:create_phase_executor`。缺失、不可导入或只返回测试 fake 的 factory 均使启动/就绪检查失败；`PhaseExecutor` 通过 lease、heartbeat、幂等键和持久化 receipt 统一承载 API 与管理员填写路径的 CLI 调用。
