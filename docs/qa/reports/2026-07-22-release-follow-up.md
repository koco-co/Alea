...
- 内置 Browser 最新复核打开 `/console/rankings`、`/console/admin/lineup`、`/console/admin/settings` 和 `/console/predictions`：无 404、无加载卡死、无客户端异常；排行榜显示真实 3 个模型与“样本不足”，阵容页的 API/CLI 两个 tab、CLI 路径/版本/认证字段均可见，预测页保持无公证记录的诚实空态。
...
| `bunx supabase@2.109.1 db reset --local --yes` | 通过，15 个迁移从空库顺序执行 |
...
| `ALEA_E2E_REAL=1 ALEA_E2E_STORAGE_STATE=/tmp/alea-e2e-local-state-fresh.json PLAYWRIGHT_BASE_URL=http://127.0.0.1 bunx playwright test --config=playwright.config.ts`（`tests/e2e`） | 20 passed，覆盖桌面 `1440×900` 与移动 `390×844` |
