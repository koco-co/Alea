...
- 连接持久化使用 `provider_execution_mode = api | codex_cli`。`api` 连接保存 HTTPS endpoint、白名单和加密 secret；`codex_cli` 连接只保存 `runtime_key=codex` 与模型/能力配置，数据库约束禁止同时出现 API URL 或 Provider secret。
...
#### 5.2.1 Codex CLI runner

Codex CLI 不在 Web、API 容器或业务仓库目录中直接执行。宿主机或生产专用 Linux 用户运行 `app.codex_runner:app`，仅暴露 `GET /health`、受 runner token 保护的 `GET /models` 与 `POST /internal/v1/execute`。请求只接受业务 prompt、固定模型 ID、超时与输出 Schema，不接受命令、二进制路径、工作目录或 shell 参数。

runner 使用参数数组和 `shell=False` 固定执行 `codex exec --json --output-schema ... --ephemeral --ignore-user-config --ignore-rules --disable plugins --sandbox read-only`。每次调用位于独立空临时目录，环境变量采用白名单，模型名拒绝前导 `-`，超时后清理整个进程组。JSONL 中出现 command/tool/MCP/Web/file-change 事件、坏 JSONL 或不符合阶段 Schema 的输出均作为失败；日志只允许 execution ID、模型、耗时、token 和脱敏错误码。

本地 `make dev` 生成一次性 runner token、启动宿主 runner 后再启动 Compose，并在退出时清理 runner。生产使用 `deploy/systemd/alea-codex-runner.service`、专用 `alea-codex` 用户和独立 `CODEX_HOME`；运维通过 `codex login --device-auth` 登录。没有目标 Linux 主机和真实 systemd smoke 证据时只能声明“配置已提供”，不能声明生产已验证。
