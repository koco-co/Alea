...
| §4、§16 | 用户/管理员 | 控制台框架 · `alea-console.html` | 登录回跳 | 九项一级导航、8 个 hover 副标题、默认落地、路由标题、消息中心、头像菜单、个人设置、角色差异 | PRD §3–4、§16 | `qa/shell-user-after-mobile.png`（临时，仅用户移动壳） | 未验收 | 一级名、8/8 hover 副标题、太玄问机默认落地和动态路由标题已静态修复；管理员移动证据仍裁切失败，Mac 锁屏阻塞新鲜双端点击与截图，个人设置与完整角色外壳仍需逐项核验 |
...
| §9.1–9.2 | 用户/管理员 | `/console/predictions` · `alea-predictions.html#today` | 主导航“太玄问机” | 今日推演、单场推演、关注、采用、停售 | PRD §9 | 待截 | 失败 | 主卡使用虚构球队/赔率/公证编号；需改为明确标注的世界杯决赛原型快照 |
...
| §13.1–13.5 | 用户/管理员 | `/console/calculator` · `alea-calculator.html` | 主导航“竞彩方案”/采用 | 桌面三栏、五玩法、过关、倍数、预算、出图、紧凑复制/下载、过期禁用 | PRD §13、DESIGN `button-icon-compact` | 待截 | 失败 | 36px 可见/44px 热区的复制与下载图标已静态落地，名称、提示和禁用原因合同齐全；独立页面仍暴露“模拟数据过期”控件，比赛、赔率和规则版本仍为无来源示例，且双端渲染/真实复制下载未验收 |
