...
const safeNext = (value: string | null) =>
  value?.startsWith("/") && !value.startsWith("//") ? value : "/console/predictions";
...
export function AuthForm({ mode }: { mode: Mode }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === "signup") {
        ...
        if (!ageConfirmed || !termsAccepted) throw new Error("请完成年龄与条款确认");
        ...
        if (!response.ok) throw new Error(result.error ?? "注册失败，请稍后再试");
  ...
  const title = mode === "login" ? "欢迎回来" : mode === "signup" ? "创建账户" : "找回密码";
  const subtitle =
    mode === "login"
      ? "登录后继续查看今天的模型推演。"
      : mode === "signup"
    ...
    <div className="auth-card">
      ...
      {mode !== "forgot" ? (
        <div className="oauth-grid">
          <button disabled={busy} type="button" className="button secondary" onClick={() => oauth("github")}>
            GitHub
          </button>
          <button disabled={busy} type="button" className="button secondary" onClick={() => oauth("google")}>
            Google
          </button>
        </div>
      ) : null}
      {mode !== "forgot" ? <div className="divider"><span>或使用邮箱</span></div> : null}
        ...
        {mode === "signup" ? (
          <>
            ...
            <label className="check-row">
              ...
              <span>我同意《风险声明与隐私条款》，并理解本平台仅提供分析研究、不提供购彩服务。</span>
            </label>
          </>
        ) : null}
        {error ? <p className="form-message error" role="alert">{error}</p> : null}
        {message ? <p className="form-message success" role="status">{message}</p> : null}
        <button className="button primary" disabled={busy || (mode === "signup" && (!ageConfirmed || !termsAccepted))} type="submit">
          {busy ? "处理中…" : mode === "login" ? "登录" : mode === "signup" ? "注册" : "发送验证邮件"}
        </button>
      </form>
      <div className="auth-links">
        {mode === "login" ? <><Link href="/forgot">忘记密码</Link><Link href="/signup">创建账户</Link></> : <Link href="/login">返回登录</Link>}
      </div>
    </div>
  );
}
