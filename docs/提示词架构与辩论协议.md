...
| 日期 | 2026-07-19 |
| 状态 | 待评审 |
| 上游依据 | PRD v1.6、TECH v1.3、`football-match-analysis` 方法论 |
...
| L2 核心方法论 | ~350 行 | 管理员在后台·系统设置编辑 | `prompt_versions` 表，版本化 |
  ...
  "plans": [
    {
      ...
      "type": "parlay",
      "legs": [
        {"match_id": "...", "play": "had", "options": ["主胜"], "odds": 1.80},
        {"match_id": "...", "play": "crs", "options": ["2:1", "1:1"], "odds": 6.50}
      ],
      "pass_types": ["2串1"],
      "multiplier": 1,
      "stake_percent": 3.0,
...
```python
def build_history_context(instance_id: str, db: Database) -> str:
...
async def build_match_context(match_id: str, sporttery: MatchDataService) -> str:
    ...
    match = await get_match(match_id)              # 先获取 match 对象
...
这个上下文**不是** AI 自己通过搜索获取的，是由 Python orchestration 层组装后注入的。AI 感知数据质量（可信度、时效与缺失状态），但感知不到底层数据源的具体 URL 或 API 密钥。AI 的唯一数据获取通道是这层上下文 + 预定义的 tools（用于补充查询）。
...
| **加权投票** | 终投按由历史综合分计算并经厂商家族归一化的 `w_i` 加权，不是简单多数 | 即使 5 个低准确率 AI 投同一个比分，高分 AI 的一票也可能更重要，同时增加同厂商实例不会线性放大厂商票权 |
                ...
                "selections": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "options": {"type": "array", "items": {"type": "string"}, "description": "支持同玩法多选（复式），单选项传单元素数组"},
                            "odds": {"type": "number"}
                        }
                    }
                },
...
AI 通过调用这些工具来获取或验证数据。所有工具的后端实现都走 MatchDataService 的降级链；AI 可感知来源标识、来源记录、可信度、采集/生效/失效时间和缺失状态，但不可见底层 URL、API 密钥或适配器内部降级细节。
...
async def run_backtest(proposal: MethodologyProposal, db: Database) -> BacktestResult:
    ...
    for sample in samples:
        # 预测器只能接收赛前冻结快照；赛果仅由评价器单独读取
        input_snapshot = load_input_snapshot(sample.input_snapshot_id)
        actual_result = load_confirmed_result(sample.result_version_id)

        old_prediction = apply_rule(input_snapshot, rule_version="current")
        if old_prediction == actual_result:
            old_correct += 1

        new_prediction = apply_rule(input_snapshot, rule_version="proposed")
        if new_prediction == actual_result:
            new_correct += 1
