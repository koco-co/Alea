...
- Use the competition label `2026 FIFA 世界杯 · 决赛` and the teams `西班牙` / `阿根廷` as the default fixture across marketing, console, prediction, calculator, and admin surfaces.
- Show the confirmed schedule as `北京时间 7 月 20 日 03:00` and the venue as `New York New Jersey Stadium`.
- A team identity lockup has three distinct slots: authoritative team/federation badge when licensed, national flag, and team name. A flag must never be styled or described as a federation badge.
- Spain and Argentina assets must share the same transparent padding, optical size, crop, border treatment, and fallback behavior.
- Do not use initial circles, emoji flags, CSS drawings, or approximate SVG crests as finished assets.

### Data Provenance Treatment

- Put provenance at page or module level instead of repeating loud “demo” badges on every row. Use a compact hairline information strip with: source, captured time, and data class.
- Confirmed match facts use neutral ink: `赛事信息 · FIFA · 采集 2026-07-19`.
- Fixed AI prototype output uses muted clay: `AI 推演数据 · 固定原型快照`.
- Unavailable Sporttery fields use amber with direct wording: `待竞彩销售数据确认`; do not render fake odds and then explain them in fine print.
- Unknown lineup, injury, referee, live-event, or result fields use the standard missing-source card: `暂缺 · 待官方确认`.
- Before the official result is confirmed, prediction cards show `待赛果确认`; hit/miss stamps are reserved for the isolated lifecycle component gallery.

### Editorial Match Workspace

- The fixture list is a compact research ledger, not a gallery of oversized rounded cards. Desktop prioritizes competition, kickoff, matchup, data state, and available actions within the first 900px.
- Keep border radii hierarchical and restrained: dense rows use hairlines and 4–8px radii; 12–16px radii are reserved for major narrative or prediction surfaces.
...
Until the real result is confirmed, the shared prototype prediction is:

- Full time prediction: `西班牙 2 : 1 阿根廷`
- Half time prediction: `1 : 0`
- Original votes: `5 / 7`
- Weighted consensus: `71%`
- Status copy: `终投预测 · 待赛果确认`
