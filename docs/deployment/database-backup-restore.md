...
3. 使用 `pg_restore --clean --if-exists --no-owner --no-acl` 恢复业务数据；不得覆盖目标环境的密钥。
...
- 先通过 8 个版本化 migration 重建数据库，再恢复 `public` 业务数据以及本地测试账户所需的 `auth.users`、`auth.identities`。
- 逻辑归档为 custom format、`--data-only --no-owner --no-acl`，共 41 个 table-data entry；归档 SHA-256 为 `175ef2a7e0ab977df0e54afb9162ae96982bb6b6b483b575843601e199e32650`。
- Storage 对象数为 0，因此对象导出与对象哈希校验记为“不适用（空桶）”，不是跳过非空对象。
- 恢复后全部 `public` 表逐表行数与源库一致；`auth.users` 行数一致。
- 两条 `source_records` 的排序哈希再次聚合后为 `dc9d474447e70cd87a49f7b222887695254e8f05b438d1e6b4aaaaaef8479928`，与源库一致。
- 恢复后的核心数据为：`matches=2`、`source_records=2`、`match_odds_snapshots=1`、`match_results=1`、待裁定冲突 `0`。
- 恢复演练命令退出码为 0；随后在恢复库执行 G1/G2 真实数据库验证，17 项通过、0 项失败、0 项跳过。
